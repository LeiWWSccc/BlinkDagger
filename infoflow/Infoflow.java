...
import soot.jimple.infoflow.sparseop.Aliasing.FlowSensitiveAliasStrategyOp;
...
import soot.jimple.infoflow.sparseop.problem.BackwardsInfoflowSpProblem;
import soot.jimple.infoflow.sparseop.problem.InfoflowSPProblem;
import soot.jimple.infoflow.sparseop.solver.BackwardsInfoflowSpSolver;
import soot.jimple.infoflow.sparseop.solver.InfoflowSparseOPSolver;
...
public class Infoflow extends AbstractInfoflow {
	...
	public void computeInfoflow(String appPath, String libPath,
        ...
        runAnalysisOp(sourcesSinks, null);
	}
	...
	public void computeInfoflow(String appPath, String libPath, String entryPoint,
		...
		// Run the analysis
        runAnalysisOp(sourcesSinks, seeds);
	}
	 ...
	 */
	private void runAnalysisOp(final ISourceSinkManager sourcesSinks, final Set<String> additionalSeeds) {
		...
		/*
			// sparse op ///////////////////////////////////////////////////////////
		*/
		BackwardsInfoflowSpProblem backProblem = null;
		InfoflowManager backwardsManager = null;
		InfoflowSparseOPSolver backSolver = null;
		...
		switch (getConfig().getAliasingAlgorithm()) {
			case FlowSensitive:
				...
				backProblem = new BackwardsInfoflowSpProblem(backwardsManager);
				backSolver = new BackwardsInfoflowSpSolver(backProblem, executor);
...
//				backSolver.setEnableMergePointChecking(true);
				...
				aliasingStrategy = new FlowSensitiveAliasStrategyOp(backwardsManager , iCfg, backSolver);

				backwardsManager.setBackwardDfgDfg(dfgSolver.getBackwardDfg());
				backwardsManager.setDfg(dfgSolver.getDfg());
				backwardsManager.setReturnInfo(dfgSolver.getReturnInfo());
		...
		InfoflowSPProblem forwardProblem  = new InfoflowSPProblem(manager,
				aliasingStrategy, zeroValue);

		// Set the options
		InfoflowSparseOPSolver forwardSolver = new InfoflowSparseOPSolver(forwardProblem, executor);
...
//		forwardSolver.setEnableMergePointChecking(true);
		...
		if (backProblem != null) {
			...
			backwardsManager.setActivationUnitsToUnits(backProblem.getActivationUnitsToUnits());
        ...
        for (SootMethod sm : getMethodsForSeeds(iCfg))
			sinkCount += scanMethodForSourcesSinksOp(sourcesSinks, forwardProblem, sm);
	...
	private int scanMethodForSourcesSinksOp(
			final ISourceSinkManager sourcesSinks,
			InfoflowSPProblem forwardProblem,
			SootMethod m) {
