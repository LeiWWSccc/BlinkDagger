...
public class AliasingOp {
	...
	public AliasingOp(IAliasingStrategy aliasingStrategy, IInfoflowCFG cfg) {
				...
				if (bases != null && !(taintedAP.isStaticFieldRef() && fieldIdx == 0)) {
					...
					for (BasePair base : bases) {
						if (base.getFields()[0] == referencedFields[fieldIdx]) {
							...
							return AccessPathFactory.v().createAccessPath(taintedAP.getDefStmt(), taintedAP.getPlainValue(),
	...
	public boolean mayAlias(Value val1, Value val2) {
		...
		if (aliasingStrategy.isInteractive())
			return aliasingStrategy.mayAlias(
					AccessPathFactory.v().createAccessPath(null, val1, false),
					AccessPathFactory.v().createAccessPath(null, val2, false));
