...
import soot.jimple.infoflow.sparseop.solver.heros.*;
 ...
 */
public class IFDSSpSolver<N,F,D extends FastSolverLinkedNode<D, N>,M,I extends BiDiInterproceduralCFG<N, M>> {
    ...
    protected static final Logger logger = LoggerFactory.getLogger(IFDSSpSolver.class);
	...
	protected final FlowFunctionsOp<N, F, D, M> flowFunctions;
	...
	protected final FlowFunctionCacheOp<N, F, D,M> ffCache;
	 ...
	 */
	public IFDSSpSolver(IFDSTabulationProblemOp<N,F,D,M,I> tabulationProblem) {
	...
	public IFDSSpSolver(IFDSTabulationProblemOp<N,F,D,M,I> tabulationProblem,
		...
		FlowFunctionsOp<N, F, D, M> flowFunctions = tabulationProblem.autoAddZero() ?
				new ZeroedFlowFunctionsOp<N,F,D,M>(tabulationProblem.flowFunctions(), zeroValue) : tabulationProblem.flowFunctions();
		if(flowFunctionCacheBuilder!=null) {
			ffCache = new FlowFunctionCacheOp<N, F,D,M>(flowFunctions, flowFunctionCacheBuilder);
	 ...
	 */
	private void processCallOp(PathEdge<N,D> edge) {
		...
		for(M sCalledProcN: callees) { //still line 14
			...
			FlowFunctionOp<F,D> function = flowFunctions.getCallFlowFunction(n, sCalledProcN);
			Set<Pair<F, D>> res = computeCallFlowFunction(function, d1, d2);

			Collection<N> startPointsOf = icfg.getStartPointsOf(sCalledProcN);
			//for each result node of the call-flow function
			for(Pair<F, D> pair : res) {
				F dfg = pair.getO1();
				D d3 = pair.getO2();
...
//				}
				...
				propagate(d3, dfg, null, d3, n, false, true); //line 15
				...
				if (endSumm != null && !endSumm.isEmpty())
					for(Pair<N, D> entry: endSumm) {
						...
						for(N retSiteN: returnSiteNs) {
							...
							FlowFunctionOp<F, D> retFunction = flowFunctions.getReturnFlowFunction(n, sCalledProcN, eP, retSiteN);
							//for each target value of the function
							for(Pair<F, D> pair1: computeReturnFlowFunction(retFunction, d3, d4, n, Collections.singleton(d1))) {
								F dfg1 = pair1.getO1();
								D d5 = pair1.getO2();
								...
								propagate(d1, dfg1, retSiteN, d5p, n, false, true);
							}
						}
					}
			}
		...
		for (N returnSiteN : returnSiteNs) {
			FlowFunctionOp<F, D> callToReturnFlowFunction = flowFunctions.getCallToReturnFlowFunction(n, returnSiteN);
			for(Pair<F, D> pair: computeCallToReturnFlowFunction(callToReturnFlowFunction, d1, d2)) {
				F dfg = pair.getO1();
				D d3 = pair.getO2();
				if (memoryManager != null)
					d3 = memoryManager.handleGeneratedMemoryObject(d2, d3);
				if (d3 != null)
					propagate(d1, dfg, returnSiteN, d3, n, false);
			}
		}
	}
	 ...
	 */
	protected Set<Pair<F, D>> computeCallFlowFunction
			(FlowFunctionOp<F, D> callFlowFunction, D d1, D d2) {
	 ...
	 */
	protected Set<Pair<F, D>> computeCallToReturnFlowFunction
			(FlowFunctionOp<F, D> callToReturnFlowFunction, D d1, D d2) {
	...
	protected void processExit(PathEdge<N,D> edge) {
		...
		if (inc != null)
			for (Entry<N,Map<D, D>> entry: inc.entrySet()) {
				...
				for(N retSiteC: icfg.getReturnSitesOfCallAt(c)) {
					//compute return-flow function
					FlowFunctionOp<F, D> retFunction = flowFunctions.getReturnFlowFunction(c, methodThatNeedsSummary,n,retSiteC);
					Set<Pair<F, D>> targets = computeReturnFlowFunction(retFunction, d1, d2, c, callerSideDs);
					//for each incoming-call value
					for(Entry<D, D> d1d2entry : entry.getValue().entrySet()) {
						final D d4 = d1d2entry.getKey();
						final D predVal = d1d2entry.getValue();

						for(Pair<F, D> pair: targets) {
							F dfg = pair.getO1();
							D d5 = pair.getO2();
							if (memoryManager != null)
							...
							propagate(d4, dfg, retSiteC, d5p, c, false, true);
						}
					}
				}
			}
		...
		if(followReturnsPastSeeds && d1 == zeroValue && (inc == null || inc.isEmpty())) {
			...
			for(N c: callers) {
				...
				for(N retSiteC: icfg.getReturnSitesOfCallAt(c)) {
					FlowFunctionOp<F, D> retFunction = flowFunctions.getReturnFlowFunction(c, methodThatNeedsSummary,n,retSiteC);
					Set<Pair<F, D>> targets = computeReturnFlowFunction(retFunction, d1, d2, c, Collections.singleton(zeroValue));
					for(Pair<F, D> pair: targets) {
						F dfg = pair.getO1();
						D d5 = pair.getO2();
						...
						if (memoryManager != null)
							d5 = memoryManager.handleGeneratedMemoryObject(d2, d5);
						if (d5 != null)
							propagate(zeroValue, dfg, retSiteC, d5, c, true, callerMethod == methodThatNeedsSummary);
					}
				}
			}
			...
			if(callers.isEmpty()) {
				FlowFunctionOp<F, D> retFunction = flowFunctions.getReturnFlowFunction(null, methodThatNeedsSummary,n,null);
	 ...
	 * @return The set of caller-side abstractions at the return site
	 */
	protected Set<Pair<F, D>> computeReturnFlowFunction
			(FlowFunctionOp<F, D> retFunction, D d1, D d2, N callSite, Collection<D> callerSideDs) {
...
//					d3 = memoryManager.handleGeneratedMemoryObject(d2, d3);
//				if (d3 != null)
//					propagate(d1, m, d3, n, false);
////					propagate(d1, m, d3, null, false);
//			}
//		}
//	}

	private void processNormalFlowOp(PathEdge<N,D> edge) {
		final D d1 = edge.factAtSource();
		final N n = edge.getTarget();
		final D d2 = edge.factAtTarget();

		for (N m : icfg.getSuccsOf(n)) {
			FlowFunctionOp<F, D> flowFunction = flowFunctions.getNormalFlowFunction(n,m);
			Set<Pair<F, D>> res = computeNormalFlowFunction(flowFunction, d1, d2);
			for (Pair<F, D> pair : res) {
				F dfg = pair.getO1();
				D d3 = pair.getO2();
				if (memoryManager != null && d2 != d3)
...
//					propagate(d1, m, d3, null, false);
	 ...
	 */
	protected Set<Pair<F, D>> computeNormalFlowFunction
			(FlowFunctionOp<F, D> flowFunction, D d1, D d2) {
	 ...
	 *        (this value is not used within this implementation but may be useful for subclasses of {@link IFDSSpSolver})
	...
	protected void propagate(D sourceVal, F dfg, N target, D targetVal,
		/* deliberately exposed to clients */ N relatedCallSite,
		/* deliberately exposed to clients */ boolean isUnbalancedReturn) {
		propagate(sourceVal, target, targetVal, relatedCallSite, isUnbalancedReturn, false);
	}
...
//	protected void propagate(D sourceVal, DataFlowNode dataFlowNode, D targetVal,
//			/* deliberately exposed to clients */ N relatedCallSite,
//			/* deliberately exposed to clients */ boolean isUnbalancedReturn,
//							 boolean forceRegister) {
//
...
//			propagate(sourceVal, (N)next.getStmt(), targetVal, relatedCallSite , isUnbalancedReturn, forceRegister);
//		}
//
//	}
	...
	static int aaa= 0;
	static String st = "$r5 = virtualinvoke $r5.<java.lang.StringBuilder: java.lang.StringBuilder append(java.lang.String)>($r2)";
...
//				if(((SootMethod)icfg.getMethodOf(edge.getTarget())).toString().contains(func)) {
//					found = true;
//					break;
//				}
			if(st.equals(edge.getTarget().toString()))
				found = true;
			if(found)
				aaa++;
