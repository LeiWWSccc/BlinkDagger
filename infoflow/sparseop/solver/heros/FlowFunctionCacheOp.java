...
public class FlowFunctionCacheOp<N, F, D, M> implements FlowFunctionsOp<N, F, D, M> {

	protected final FlowFunctionsOp<N, F, D, M> delegate;

	protected final LoadingCache<NNKey, FlowFunctionOp<F,D>> normalCache;

	protected final LoadingCache<CallKey, FlowFunctionOp<F,D>> callCache;

	protected final LoadingCache<ReturnKey, FlowFunctionOp<F,D>> returnCache;

	protected final LoadingCache<NNKey, FlowFunctionOp<F,D>> callToReturnCache;

    private final Logger logger = LoggerFactory.getLogger(getClass());

	@SuppressWarnings("unchecked")
	public FlowFunctionCacheOp(final FlowFunctionsOp<N, F, D, M> delegate, @SuppressWarnings("rawtypes") CacheBuilder builder) {
		this.delegate = delegate;
		
		normalCache = builder.build(new CacheLoader<NNKey, FlowFunctionOp<F, D>>() {
			public FlowFunctionOp<F, D> load(NNKey key) throws Exception {
				return delegate.getNormalFlowFunction(key.getCurr(), key.getSucc());
			}
		});
		
		callCache = builder.build(new CacheLoader<CallKey, FlowFunctionOp<F, D>>() {
			public FlowFunctionOp<F, D> load(CallKey key) throws Exception {
				return delegate.getCallFlowFunction(key.getCallStmt(), key.getDestinationMethod());
			}
		});
		
		returnCache = builder.build(new CacheLoader<ReturnKey, FlowFunctionOp<F, D>>() {
			public FlowFunctionOp<F, D> load(ReturnKey key) throws Exception {
				return delegate.getReturnFlowFunction(key.getCallStmt(), key.getDestinationMethod(), key.getExitStmt(), key.getReturnSite());
			}
		});
		
		callToReturnCache = builder.build(new CacheLoader<NNKey, FlowFunctionOp<F, D>>() {
			public FlowFunctionOp<F, D> load(NNKey key) throws Exception {
