 ...
 *
 */
public class PropagationRuleManagerOp {
	...
	public PropagationRuleManagerOp(InfoflowManager manager, Aliasing aliasing,
...
//			ruleList.add(new TypingPropagationRule(manager, aliasing, zeroValue, results));
	 ...
	 */
	public Set<Pair<DataFlowNode, Abstraction>> applyNormalFlowFunction(Abstraction d1,
	...
	public Set<Pair<DataFlowNode, Abstraction>> applyNormalFlowFunction(Abstraction d1,
		...
		Set<Pair<DataFlowNode, Abstraction>> res = null;
		if (killSource == null)
			killSource = new ByReferenceBoolean();
		for (ITaintPropagationRule rule : rules) {
			Collection<Pair<DataFlowNode, Abstraction>> ruleOut = rule.propagateNormalFlow(d1,
					source, stmt, destStmt, killSource, killAll);
			if (killAll != null && killAll.value)
				return null;
			if (ruleOut != null && !ruleOut.isEmpty()) {
				if (res == null)
					res = new HashSet<Pair<DataFlowNode, Abstraction>>(ruleOut);
				else
					res.addAll(ruleOut);
			}
		}
		...
		if ((killAll == null || !killAll.value) && !killSource.value) {
			if (res == null) {
				res = new HashSet<>();
				res.add(new Pair<DataFlowNode, Abstraction>(null, source));
			}
			else
				res.add(new Pair<DataFlowNode, Abstraction>(null, source));
	 ...
	 */
	public Set<Pair<DataFlowNode, Abstraction>> applyCallFlowFunction(Abstraction d1,
			Abstraction source, Stmt stmt, SootMethod dest, ByReferenceBoolean killAll) {
		Set<Pair<DataFlowNode, Abstraction>> res = null;
		for (ITaintPropagationRule rule : rules) {
			Collection<Pair<DataFlowNode, Abstraction>> ruleOut = rule.propagateCallFlow(
	 ...
	 * @return The collection of outgoing taints
	 */
	public Set<Pair<DataFlowNode, Abstraction>> applyCallToReturnFlowFunction(Abstraction d1,
	...
	public Set<Pair<DataFlowNode, Abstraction>> applyCallToReturnFlowFunction(Abstraction d1,
		...
		Set<Pair<DataFlowNode, Abstraction>> res = null;
		for (ITaintPropagationRule rule : rules) {
			Collection<Pair<DataFlowNode, Abstraction>> ruleOut = rule.propagateCallToReturnFlow(
	 ...
	 * @return The collection of outgoing taints
	 */
	public Set<Pair<DataFlowNode, Abstraction>> applyReturnFlowFunction(
			Collection<Abstraction> callerD1s, Abstraction source, Stmt stmt,
			Stmt retSite, Stmt callSite, ByReferenceBoolean killAll) {
		Set<Pair<DataFlowNode, Abstraction>> res = null;
		for (ITaintPropagationRule rule : rules) {
			Collection<Pair<DataFlowNode, Abstraction>> ruleOut = rule.propagateReturnFlow(callerD1s,
