	...
	public Collection<Pair<DataFlowNode, Abstraction>> propagateCallToReturnFlow(Abstraction d1,
		...
		if (wrapperTaints != null) {
			...
			for (Abstraction wrapperAbs : wrapperTaints) {
				...
				DataFlowNode dfg = useApTofindDataFlowGraph(wrapperAbs.getAccessPath(), stmt);
				if(dfg == null)
					 dfg = useBaseTofindDataFlowGraph(wrapperAbs.getAccessPath().getPlainValue(), stmt);
				...
				if(dfg.getSuccs() != null){
					...
					for(DataFlowNode next : nextSet) {
						res.add(new Pair<>(next, wrapperAbs));
					}
				}
