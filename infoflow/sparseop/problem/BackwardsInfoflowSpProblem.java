...
import soot.jimple.infoflow.sparseop.solver.heros.IdentityOp;
 ...
 */
public class BackwardsInfoflowSpProblem extends AbstractInfoflowProblemOp {
	...
	public BackwardsInfoflowSpProblem(InfoflowManager manager) {
		super(manager);
	}
	...
	public FlowFunctionsOp<Unit, DataFlowNode, Abstraction, SootMethod> createFlowFunctionsFactory() {
		return new FlowFunctionsOp<Unit, DataFlowNode, Abstraction, SootMethod>() {
			 ...
			 */
			private Set<Pair<DataFlowNode, Abstraction>> computeAliases
				...
				final Set<Pair<DataFlowNode, Abstraction>> res = new HashSet<>();
				...
				if (!leftSideMatches) {
					...
					Set<Unit> nextSet = getNextStmtFromDfg(leftValue, defStmt, source);
					for (Unit u : nextSet)
				...
				if (!aliasOverwritten && !(rightValue.getType() instanceof PrimType)) {
					...
					if (rightValue instanceof InstanceFieldRef) {
						...
						if (newLeftAbs == null)
							newLeftAbs = checkAbstraction(source.deriveNewAbstraction(
									source.getAccessPath().copyWithNewValue(defStmt, leftValue, newType, false), defStmt));
					}
					...
					if (newLeftAbs != null) {
						...
						if (!newLeftAbs.getAccessPath().equals(source.getAccessPath())) {
							...
							res.addAll(getNextResFromBackwardDfg(leftValue, defStmt, newLeftAbs));
							// Propagate the new alias upwards
							//res.add(newLeftAbs);

							Set<Unit> nextSet = getNextStmtFromDfg(leftValue, defStmt, newLeftAbs);
							for (Unit u : nextSet)
					...
					if (addRightValue) {
						...
						if (addRightValue) {
							...
							if (newAbs != null && !newAbs.getAccessPath().equals(source.getAccessPath())) {
								res.addAll(getNextResFromBackwardDfg(rightValue, defStmt, newAbs));

								//res.add(newAbs);

								Set<Unit> nextSet = getNextStmtFromDfg(rightValue, defStmt, newAbs);
								for (Unit u : nextSet)
...
//									fSolver.processEdge(new PathEdge<Unit, Abstraction>(d1, u, newAbs));
			...
			private Set<Unit> getNextStmtFromDfg(Value value , Unit stmt, Abstraction abs) {
				...
				DataFlowNode dataFlowNode = useValueTofindDataFlowGraph(value, stmt, true);
				AccessPath ap = abs.getAccessPath();
				SootField firstField = ap.getFirstField();
			...
			public FlowFunctionOp<DataFlowNode, Abstraction> getNormalFlowFunction(final Unit src, final Unit dest) {
				...
				if (src instanceof DefinitionStmt) {
					...
					return new SolverNormalFlowFunctionOp() {
						...
						public Set<Pair<DataFlowNode, Abstraction>> computeTargets(Abstraction d1, Abstraction source) {
							...
							Set<Pair<DataFlowNode, Abstraction>> res = computeAliases(defStmt, leftValue, d1, source);
			...
			public FlowFunctionOp<DataFlowNode, Abstraction> getCallFlowFunction(final Unit src, final SootMethod dest) {
				...
				return new SolverCallFlowFunctionOp() {
					...
					public Set<Pair<DataFlowNode, Abstraction>> computeTargets(Abstraction d1, Abstraction source) {
						...
						Set<Pair<DataFlowNode, Abstraction>> res = new HashSet<Pair<DataFlowNode, Abstraction>>();
											...
											if (manager.getTypeUtils().checkCast(source.getAccessPath(), rStmt.getOp().getType())) {
												Abstraction abs = checkAbstraction(source.deriveNewAbstraction
														(source.getAccessPath().copyWithNewValue
																(null, rStmt.getOp(), null, false), (Stmt) src));
												if (abs != null) {
													Set<Pair<DataFlowNode, Abstraction>> ret = useValueTofindCallInfo(rStmt.getOp(), rStmt, abs);
													res.addAll(ret);
												}
												//	res.add(abs);
											}
...
//						}
						...
						if (!isExecutorExecute
							...
							if (iIExpr.getBase() == sourceBase && manager.getTypeUtils().hasCompatibleTypesForCall(
								...
								if (!param) {
									Abstraction abs = checkAbstraction(source.deriveNewAbstraction
											(source.getAccessPath().copyWithNewValue(null, thisLocal), (Stmt) src));
									if (abs != null) {

										for(Unit sp : manager.getICFG().getStartPointsOf(dest)) {
											Set<Pair<DataFlowNode, Abstraction>> ret = useValueTofindCallInfo(thisLocal, sp, abs);
											res.addAll(ret);
										}
						...
						if (isExecutorExecute) {
							if (ie.getArg(0) == source.getAccessPath().getPlainValue()) {
								Abstraction abs = checkAbstraction(source.deriveNewAbstraction
										(source.getAccessPath().copyWithNewValue(null, thisLocal), stmt));
								if (abs != null) {
							...
							for (int i = 0; i < ie.getArgCount(); i++) {
								if (ie.getArg(i) == source.getAccessPath().getPlainValue()) {
									Abstraction abs = checkAbstraction(source.deriveNewAbstraction(
											source.getAccessPath().copyWithNewValue(null, paramLocals[i]), stmt));
									if (abs != null) {
										for(Unit sp : manager.getICFG().getStartPointsOf(dest)) {
											Set<Pair<DataFlowNode, Abstraction>> ret = useValueTofindCallInfo(paramLocals[i], sp, abs);
											res.addAll(ret);
										}
			...
			public Set<Pair<DataFlowNode, Abstraction>> useValueTofindCallInfo(Value value, Unit stmt, Abstraction abs) {

				Set<Pair<DataFlowNode, Abstraction>> res = new HashSet<>();

				DataFlowNode dataFlowNode = useValueTofindDataFlowGraph(value, stmt, false);
				if(dataFlowNode != null && dataFlowNode.getSuccs() != null) {
					for(Set<DataFlowNode> nexts : dataFlowNode.getSuccs().values()) {
						for(DataFlowNode n : nexts) {
							if(n.getValue() != null && n.getValue().equals(value)) {
								res.add(new Pair<>(n, abs));
							}
			...
			public Set<Pair<DataFlowNode, Abstraction>> useValueTofindReturnInfo(Value value, Unit stmt, Abstraction abs) {
...
//			public Set<DataFlowNode> useBaseTofindReturnInfo(Value base, SootField field, Unit stmt, Set<DFGEntryKey> keySet) {
//
//				Set<DataFlowNode> res = new HashSet<>();
//
//				for(DFGEntryKey key : keySet) {
//					Set<DataFlowNode> ret = useBaseTofindReturnInfo(base, field, stmt, key);
//					if(ret != null) {
//						res.addAll(ret);
//					}
//				}
...
//
//
//			public Set<DataFlowNode> useBaseTofindReturnInfo(Value base, SootField field, Unit stmt, DFGEntryKey key) {
//				SootMethod caller = manager.getICFG().getMethodOf(stmt);
//
//				if(!manager.getReturnInfo().containsKey(caller)) return null;
//				Map<Value, Map<DFGEntryKey, Set<DataFlowNode>>> l1 = manager.getReturnInfo().get(caller);
//				if(!l1.containsKey(base)) return null;
//				Map<DFGEntryKey, Set<DataFlowNode>> l2 = l1.get(base);
//				if(!l2.containsKey(key)) return null;
//				Set<DataFlowNode> ret = l2.get(key);
...
//			}
			...
			@Override
			public FlowFunctionOp<DataFlowNode, Abstraction> getReturnFlowFunction(final Unit callSite, final SootMethod callee,
...
//							return notifyOutFlowHandlers(exitStmt, d1, source,
//									Collections.singleton(new Pair<DataFlowNode, Abstraction>(null, source)),
//									FlowFunctionType.ReturnFlowFunction);
//						}
						...
						if (isExecutorExecute) {
							...
							if (source.getAccessPath().getPlainValue() == thisLocal) {
								Abstraction abs = checkAbstraction(source.deriveNewAbstraction
										(source.getAccessPath().copyWithNewValue(callSite, ie.getArg(0)), (Stmt) exitStmt));
								if (abs != null) {

									res.addAll(useValueTofindReturnInfo(ie.getArg(0), callSite, abs));
									//res.add(abs);
							...
							for (int i = 0; i < paramLocals.length; i++) {
								if (paramLocals[i] == sourceBase) {
									...
									if (callSite instanceof Stmt) {
										...
										Abstraction abs = checkAbstraction(source.deriveNewAbstraction
												(source.getAccessPath().copyWithNewValue(callSite, originalCallArg), (Stmt) exitStmt));
										if (abs != null) {
											res.addAll(useValueTofindReturnInfo(originalCallArg, callSite, abs));
									...
									if (!parameterAliases) {
										if (callSite instanceof Stmt) {
											...
											if (stmt.getInvokeExpr() instanceof InstanceInvokeExpr) {
												InstanceInvokeExpr iIExpr = (InstanceInvokeExpr) stmt.getInvokeExpr();
												Abstraction abs = checkAbstraction(source.deriveNewAbstraction
														(source.getAccessPath().copyWithNewValue(callSite, iIExpr.getBase()), (Stmt) exitStmt));
												if (abs != null) {
													res.addAll(useValueTofindReturnInfo(iIExpr.getBase(), callSite, abs));
			...
			@Override
			public FlowFunctionOp<DataFlowNode, Abstraction> getCallToReturnFlowFunction(final Unit call, final Unit returnSite) {
...
//							}
						...
						return notifyOutFlowHandlers(call, d1, source,
								Collections.singleton(new Pair<DataFlowNode, Abstraction>(null, source)),
								FlowFunctionType.CallToReturnFlowFunction);
					}
				};
