package soot.jimple.infoflow.sparseop.rules;
 ...
 * @author wanglei
 */
public class SourcePropagationRuleOP extends AbstractTaintPropagationRule {


    public SourcePropagationRuleOP(InfoflowManager manager, Aliasing aliasing,
        ...
        if (source == getZeroValue()) {
            ...
            if (sourceInfo != null && !sourceInfo.getAccessPaths().isEmpty()) {
                ...
                for (AccessPath ap : sourceInfo.getAccessPaths()) {
                    ...
                    DataFlowNode dfn = useApTofindDataFlowGraph(ap, stmt);
...
//                        abs.setCorrespondingCallSite(stmt);
    ...
    public DataFlowNode useApTofindDataFlowGraph(AccessPath ap, Stmt stmt) {
        SootMethod caller = getManager().getICFG().getMethodOf(stmt);
        Value base = ap.getPlainValue();
        Pair<BaseInfoStmt, DataFlowNode> pair = getManager().getDfg().
                get(caller).get(base).get(new Pair<Unit, Value>(stmt, base));
