									...
									if (u instanceof ReturnStmt) {
										...
										if (rStmt.getOp() instanceof Local
											...
											if (manager.getTypeUtils().checkCast(source.getAccessPath(), rStmt.getOp().getType())) {
												...
												if (abs != null) {
													Set<Pair<DataFlowNode, Abstraction>> ret = useValueTofindReturnInfo(rStmt.getOp(), rStmt, abs);
													res.addAll(ret);
												}
												//	res.add(abs);
											}
								...
								if (!param) {
									...
									if (abs != null) {
										...
										for(Unit sp : manager.getICFG().getStartPointsOf(dest)) {
											Set<Pair<DataFlowNode, Abstraction>> ret = useValueTofindReturnInfo(thisLocal, sp, abs);
											res.addAll(ret);
										}
						...
						if (isExecutorExecute) {
							...
							for (int i = 0; i < ie.getArgCount(); i++) {
								if (ie.getArg(i) == source.getAccessPath().getPlainValue()) {
									...
									if (abs != null) {
										for(Unit sp : manager.getICFG().getStartPointsOf(dest)) {
											Set<Pair<DataFlowNode, Abstraction>> ret = useValueTofindReturnInfo(paramLocals[i], sp, abs);
											res.addAll(ret);
										}
