...
public class WrapperPropagationRule extends AbstractTaintPropagationRule {
	 ...
	 */
	private Set<Pair<DataFlowNode, Abstraction>> computeWrapperTaints
			(Abstraction d1,
			final Stmt iStmt,
			Abstraction source) {
		...
		Set<Pair<DataFlowNode, Abstraction>> res = new HashSet<>();
				//getManager().getTaintWrapper().getTaintsForMethod(iStmt, d1, source);
//		if(res != null) {
...
//		}
	...
	public Collection<Pair<DataFlowNode, Abstraction>> propagateCallToReturnFlow(Abstraction d1,
		...
		Collection<Pair<DataFlowNode, Abstraction>> wrapperTaints = computeWrapperTaints(d1, stmt, source);
